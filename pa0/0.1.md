...
{% panel style="info", title="Linux and Linux?" %}

Debian and Ubuntu are both kinds of Linux. What are the common and different points betweent them? Please search the Internet and answer this.
...
4. **Memory size.** Just leaving the default size is okay, but we recommend you to set an value not less than `1024MB` or you may feel that the machine is running slowly. Then click `Next`.
   ...
   {% panel style="info", title="Can't memory be larger?" %}

   Can the size of memory be as large as possible? There's a limitation of 4096MB for 32-bit operating systems, why? (Hint: Architecture and addressing)
...
> 因为国内网络环境不佳，安装程序会自动连接到国外的下载点下载一些不必要的更新，然而这个连接往往不能正常建立，这样以来就会花费很多时间反复进行很多失败的网络请求，导致正常的安装流程阻塞，耽误同学们的宝贵时间，所以请务必先断网，再继续本节的安装过程！虽然这样的安装流程将导致首次开机没有网络，但是我们会在下一节中首先带领同学们解决该问题。
