...
You may get the latest version of Debian [here on the official website](https://www.debian.org/releases/stable/debian-installer/). But we recommend you to use a mirror source in the mainland of China, for some reasons you might be already aware of. You may download one in the following mirror list:
...
4. **Memory size.** Just leaving the default size is okay, but we recommend you to set an value not less than `1024MB` or you may feel that the machine is running slowly. Then click `Next`.
   ...
   ```
   By the way, this size is not the larger the better, for there's a limitation of 4096MB, why?(Hint: Architecture and addressing)
   ```
...
1. Select the newly created VM on the sidebar, and choose `Settings` on the toolbar.
...
6. Select the `Graphical install` using up and down keys on the keyboard, and press Enter.
...
5. Just wait for a moment. **Configure the network.** 

   1. **Hostname**: just leave the default settings (`debian`).
   2. **Domain name**:  just leave the default settings (empty).

6. **After configuring the network, please cut down the Internet connection of VM or you will spend much time waiting for the updates!** To cut down the connection, please right click the icon which seems like two screens on the right bottom of the status bar then click `Connect Network Adapter ` and you will see the icon become disabled.

   ```
   因为国内网络环境不佳，安装程序会自动连接到国外的下载点下载一些不必要的更新，然而这个连接往往不能正常建立，这样以来就会花费很多时间反复进行很多失败的网络请求，导致正常的安装流程阻塞，耽误同学们的宝贵时间，所以请务必先断网，再继续本节的安装过程！
   此外，请务必在设置好网络以后再断开网络，这样虚拟机才能够自动配置好网络，以免安装完成进入系统后无法正常联网。
   ```
...
7. **Set up users and passwords.** Here we are going to create accounts of the system, it's normally recommended to set complex passwords but here we don't force you to do that for we are just for experimental usage.
   ...
   2. **Full name for the new user**: to trace the process of the experiment, we make an agreement that your username should be set as your own real name. For example, `zhangsan` for `张三` or `Wangermazi` for `王二麻子`, etc.
      ...
      **注意：为了保证大家的实验是自己独立完成，我们要求大家使用自己真名的汉语拼音作为非 root 用户的用户名进行实验，并在实验报告中出现的截图中加以体现。如果有同学是本来就使用 Linux 操作系统作为桌面，可以新建一个满足上述要求的用户名的新用户进行实验。**
...
10. **Install the base system.** Just wait.
    ...
    If by accident you meet the question `Scan another CD or DVD?`, please select `No` and  click `Continue`.
...
11. **Configure the package manager.** We don't need to use the mirrors provided for network issues, we will configure it in the future so do the following steps:
    ...
    1. **Debian archive mirror country**: don't choose any one in the list and click `Go Back`.
    2. **Continue without a network mirror?** Select `Yes` and click `Continue`.
    3. Just wait for the following configuration. **If the process is blocked when configuring apt, you can turn on the network manually. After this step, please turn it down again.**

12. **Participate in the package usage survey?** Any choice will be fine.

13. **Software selection.** Choose the `standard system utilities` and click `Continue`. Then just wait.
...
15. **Finish the installation.**  Click `Continue` and the VM will remove the CD automatically then reboot the VM.
