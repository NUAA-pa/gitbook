...
你可能已经发现, `opcode_table` 数组中大多数元素在一开始都初始化为了 `EMPTY`. 从上述宏定义可以看到, EMPTY 事实上也是一个宏, 而且可以进行展开. 不兜圈子了, 事实上, `EMPTY` 与 `EX(?)` 和 `EXW(?, ?)` 都是等价的. 那么机智的你, 能否推断出这里的?是什么呢?

{% endpanel %}

{% panel style="success", title="画蛇添足" %}

`mov` 指令后面的`l`，`b`，`w` 这些字符为什么会存在？仔细观察他们是在什么地方出现的，不加这些标识符会有什么后果？每个指令，准确地说，每个指令每次出现都需要用着几个字符标识吗？
...
以上是 PA2.2 的所有内容. 
>>>>>>> 49537df18982354c06be1e4d4febde17fc3bb37f
